...
namespace calib
{
  struct PoseDrawer
  {
    ...
    draw(cv::Mat& drawImage, const cv::Mat& K, const cv::Mat& R, const cv::Mat T)
    {
      ...
      int baseline = 0;
      Size sz = cv::getTextSize(scaleText, cv::FONT_HERSHEY_SIMPLEX, 1, 1, &baseline);
      ...
      putText(drawImage, scaleText, box_origin, cv::FONT_HERSHEY_SIMPLEX, 1.0, c[0], 1, cv::LINE_AA, false);
      putText(drawImage, "Z", ip[3], cv::FONT_HERSHEY_SIMPLEX, 1.0, c[3], 1, cv::LINE_AA, false);
      putText(drawImage, "Y", ip[2], cv::FONT_HERSHEY_SIMPLEX, 1.0, c[2], 1, cv::LINE_AA, false);
      putText(drawImage, "X", ip[1], cv::FONT_HERSHEY_SIMPLEX, 1.0, c[1], 1, cv::LINE_AA, false);
